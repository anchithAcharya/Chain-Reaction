...
void play()
{
	...
	do
	{
		...
		if(p->player=='A' || p->player=='B' || p->player==16)
			{
				p->player+=32;
				a=aa;
				continue;
...
void get_INPUT(cell grid[ROW][COL],bool a)
{
    int temp1; char dir='0',temp2; bool z=1;
...
{
	bool unstable=0; cellptr a=NULL;
	...
	while(unstable)
	{
		...
		for(ptr=grid;ptr<=&grid[ROW-1][COL-1];ptr++)
		{

			if(ptr->v1>ptr->threshold)
			{
...
void save(cell grid[ROW][COL],char t)
{
	...
	for(i=0;i<ROW;i++)
    {
        for(j=0;j<COL;j++)
        {
			...
			grid[i][j].u1=grid[i][j].orbs;
			grid[i][j].u2=grid[i][j].player;
...
void undo(cell grid[ROW][COL])
{
	...
	for(i=0;i<ROW;i++)
    {
        for(j=0;j<COL;j++)
        {
				grid[i][j].orbs=grid[i][j].u1;
				grid[i][j].player=grid[i][j].u2;
        }
    }
